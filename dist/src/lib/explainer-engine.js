...
function deterministicFallback(prompt, niche) {
...
export async function generateExplainer({ prompt, niche }) {
    ...
    try {
        if (OPENAI_KEY) {
            ...
            const text = data?.choices?.[0]?.message?.content ?? data?.choices?.[0]?.text ?? String(data ?? '');
