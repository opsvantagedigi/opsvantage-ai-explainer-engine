Applying the generated SQL migration to Supabase

Files created:
- `prisma_migration.sql` — SQL created from Prisma schema, contains enums, tables, indexes, and FKs.

Options to apply the SQL to your Supabase database:
...
2) psql (command line)
- From your machine with network access to Supabase, run:

```bash
psql "postgresql://postgres:YOUR_PASSWORD@db.sseikdmbfnyotsqugtgm.supabase.co:5432/postgres" -f prisma_migration.sql
```

3) pgAdmin or TablePlus
- Connect to your Supabase Postgres using the connection string.
