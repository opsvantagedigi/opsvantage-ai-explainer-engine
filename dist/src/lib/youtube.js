...
const oauth2ClientFactory = () => new google.auth.OAuth2(process.env.YOUTUBE_CLIENT_ID, process.env.YOUTUBE_CLIENT_SECRET, process.env.YOUTUBE_REDIRECT_URI);
...
export async function handleOAuthCallback(code, workspaceId) {
    const oauth2Client = oauth2ClientFactory();
...
export async function uploadShort(workspaceId, shortVideoId) {
